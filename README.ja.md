...
- プレビュー横で main スキンと読み込まれた include/CSV ファイルを同時に編集し、編集中にプレビューを更新できます。
- プレビューサイズは `#RESOLUTION` を優先し、存在しない場合は `#INFORMATION` の x/y 値を使用します。
- read-only mode ではプレビューをドラッグしてパンし、マウスホイールまたは `-`/`=` キーでズームできます。
